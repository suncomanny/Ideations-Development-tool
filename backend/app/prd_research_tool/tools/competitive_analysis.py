...
def is_placeholder_or_guidance(value: str | None) -> bool:
    text = normalize_text(value)
    if not text:
        return True
    lowered = text.lower()
    if lowered in {"n/a", "na", "none", "no", "tbd", "not applicable"}:
        return True
    if lowered.startswith(("n/a", "na -", "tbd", "not applicable")):
        return True
...
def first_instruction_clause(value: str) -> str:
    return re.split(
        r";|\bvalidate\b|\bconfirm\b|\bunless\b|\bif\b|\btarget\b|\btbd\b",
        value,
        maxsplit=1,
        flags=re.IGNORECASE,
    )[0].strip(" ,.-")
...
def clean_feature_labels(value: Any) -> list[str]:
    text = normalize_text(value)
    if not text or is_placeholder_or_guidance(text):
        return []

    lowered = text.lower()
    bulb_bases = re.findall(r"\bE(?:12|17|26|39)\b", text, flags=re.IGNORECASE)
    if bulb_bases:
        return unique_preserve_order([f"{base.upper()} socket" for base in bulb_bases])
    if lowered.startswith("dry") and "only if" in lowered:
        return []
    if lowered in {"dry", "dry rated", "indoor", "indoor use"}:
        return []
    if "decorative indoor fixture" in lowered or "omnidirectional lamping" in lowered:
        return []
    if "bulbs are included" in lowered or "optional" in lowered:
        return []

    cleaned = first_instruction_clause(text)
    if not cleaned or is_placeholder_or_guidance(cleaned):
        return []
    if len(cleaned.split()) > 7 and not any(term in cleaned.lower() for term in ["dimm", "sensor", "smart", "linkable"]):
        return []
    return [cleaned]


def clean_certification_labels(values: list[Any] | None) -> list[str]:
    output: list[str] = []
    for value in values or []:
        ...
        for label, pattern in known_patterns:
            if re.search(pattern, lowered, flags=re.IGNORECASE):
                output.append(label)
                matched = True
        if not matched and not any(term in lowered for term in ["validate", "target", "supplier", "listing"]):
            output.append(first_instruction_clause(text))
...
def coverage_evidence_strength(
    *,
    ...
    direct_spec_count: int = 0,
) -> str:
    """Estimate how trustworthy a surfaced coverage signal is."""
    minimum_spec_sample = max(4, math.ceil(total_count * 0.25))
    if matched_count <= 0:
        if direct_spec_count < minimum_spec_sample:
            return "weak"
...
def coverage_evidence_note(
    *,
    ...
    if matched_count <= 0:
        if evidence_strength == "weak" and direct_spec_count < max(4, math.ceil(total_count * 0.25)):
            return "Absence is provisional because collected items are mostly market seeds, not spec-rich source pages."
...
def feature_match_detail(item: dict[str, Any], label: str) -> dict[str, Any]:
    ...
    if "switching time" in lowered:
        if "switching time" in haystack or "instant on" in haystack:
            return explicit()
        return {"matched": False}
    socket_match = re.fullmatch(r"(e(?:12|17|26|39))(?: socket| base)?", lowered)
    if socket_match:
        base = socket_match.group(1)
...
def has_direct_spec_detail(item: dict[str, Any]) -> bool:
    """Return whether a normalized candidate carries source-page spec detail."""
    if item.get("collection_method") not in {None, "", "stackline_seed"}:
        return True
    if item.get("features") or item.get("certifications"):
        return True
    for key in ["dimmable", "dimming_type", "wattage", "lumens", "cct", "cri", "voltage"]:
...
def build_spec_coverage(packet: dict[str, Any], items: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    certifications = unique_preserve_order(
        [
            label
            for value in as_list(must_validate.get("certifications"))
            for label in clean_certification_labels([value])
        ]
    )
    feature_watchlist = []
