...
family_rollup AS (
    SELECT
        ...
        string_agg(DISTINCT COALESCE(NULLIF(pf.po_vendors, ''), NULLIF(pb.shopify_vendor, '')), ', ') AS vendor,
        COALESCE(MIN(pb.product_vendor_cost), MIN(pf.po_unit_cost)) AS vendor_cost,
        CASE
    ...
    FROM product_base pb
    LEFT JOIN po_facts pf ON pf.product_id = pb.product_id
...
SELECT
    ...
    fr.product_url,
    fr.image_url,
    'Postgres query generated by Step 1 line review module' AS source_notes
...
def write_line_review_sheet(workbook, context: LineReviewContext) -> None:
    ...
    if context.rows:
        ...
        ws.cell(2, 1).value = "(no approved DB line review snapshot found)"
