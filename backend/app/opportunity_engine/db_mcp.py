...
class McpRemoteClient:
    def __init__(self, server_url: str = POSTGRES_MCP_URL, timeout_seconds: int = 120, client_name: str = "sunco-ideation-development") -> None:
...
def sanitize_mcp_error(exc: Exception) -> str:
    text = str(exc)
    text = re.sub(r"(token|authorization|password|secret)=([^;)'\\s]+)", r"\1=***", text, flags=re.IGNORECASE)
    return text
