...
def audit_category_profiles(root: Path | str, limit: int = 12) -> Path:
    ...
    for category in _load_categories(root):
        ...
        try:
            evidence_rows, _source = execute_redshift_sql(
                build_ecommerce_sql(category.slug, limit=limit),
                timeout_seconds=90,
                client_name="sunco-profile-audit-redshift",
            )
            ...
            result["status"] = "error"
            result["error"] = f"{type(exc).__name__}: {sanitize_redshift_error(exc)}"
        rows.append(result)
