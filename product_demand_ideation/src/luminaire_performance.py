...
def normalize_luminaire_performance(*texts: Any) -> LuminairePerformance:
    ...
    if max_lumens and max_watts:
        ...
        efficiency_note = f"Treat {max_lumens:,}lm as the user-facing brightness target; wattage/load target still needs validation"
    elif max_watts:
        efficiency_note = f"Treat {max_watts}W as the installer/load target; lumen brightness target still needs validation"
