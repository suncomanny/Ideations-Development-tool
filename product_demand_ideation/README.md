...
Normal `1B` workbook runs read from that local cache. If the cache does not exist yet, the tool will try to seed it from Postgres MCP and then continue. For Sunco's current catalog change rate, a weekly or monthly refresh is enough for routine A/B testing; refresh before leadership readouts or after known catalog/product-status updates.
